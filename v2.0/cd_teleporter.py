...
class TeleportEngine:
    ...
    def detach(self):
        ...
        if self.block:
            k32.VirtualFreeEx(
                self.pm.process_handle, self.block, 0, MEM_RELEASE)
            self.block = 0
        if self.pm:
        ...
        for offset in range(0x10000, 0x7FFF0000, 0x10000):
            for addr in [near + offset, near - offset]:
                ...
                if result:
                    ...
                    return
        raise RuntimeError("Could not allocate memory near game module")
...
class TeleporterApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Map window state
        self._map_image = None       # PIL Image of worldmap (full res)
        self._map_photo = None       # ImageTk.PhotoImage for canvas
        self._map_zoom = 1.0
        self._map_center = None      # (game_x, game_z) zoom center
        self._map_dest = None        # (abs_x, abs_y, abs_z) clicked destination
        self._map_visible = False
        self._map_window = None
        self._map_disp = None        # (off_x, off_y, disp_w, disp_h, x0, y0, vw, vh)
    ...
    def _build_ui(self):
        ...
        ttk.Label(self._status_frame, textvariable=self.status_var,
        ...
        ttk.Button(map_btn_frame, text="Save", style='Small.TButton',
                   command=self._save_map_marker).pack(side=tk.LEFT, padx=(0, 3))
        ...
        # ── WORLD MAP (separate window, opened via toggle button) ─────
        if _HAS_PIL:
            self._map_window = None  # created on first toggle
    ...
    def _build_map_window(self):
        """Build the interactive world map in a separate Toplevel window."""
        win = tk.Toplevel(self)
        win.title("World Map")
        win.geometry("1350x1125")
        win.minsize(700, 500)
        win.configure(bg=self.BG)
        win.protocol("WM_DELETE_WINDOW", self._toggle_map)
        self._map_window = win

        # Apply dark title bar to map window too
        try:
            hwnd = int(win.wm_frame(), 16)
            val = ctypes.c_int(1)
            ctypes.windll.dwmapi.DwmSetWindowAttribute(
                hwnd, 20, ctypes.byref(val), ctypes.sizeof(val))
            color = ctypes.c_int(int(self.BG.lstrip('#'), 16))
            color_bgr = ctypes.c_int(
                ((color.value & 0xFF) << 16)
                | (color.value & 0xFF00)
                | ((color.value >> 16) & 0xFF))
            ctypes.windll.dwmapi.DwmSetWindowAttribute(
                hwnd, 35, ctypes.byref(color_bgr), ctypes.sizeof(color_bgr))
        except Exception:
            pass
        ...
        # Top bar: destination coords (read-only) + buttons
        top = ttk.Frame(win)
        top.pack(fill=tk.X, padx=8, pady=(8, 4))

        ttk.Label(top, text="Destination:", font=('Segoe UI', 9, 'bold')).pack(
            side=tk.LEFT, padx=(0, 6))

        self._map_dest_x_var = tk.StringVar(value="---")
        self._map_dest_z_var = tk.StringVar(value="---")
        self._map_height_var = tk.StringVar(value=f"{self._map_height:.0f}")
        for label, var in [("X:", self._map_dest_x_var),
                           ("Z:", self._map_dest_z_var),
                           ("Height:", self._map_height_var)]:
            ttk.Label(top, text=label).pack(side=tk.LEFT, padx=(4, 0))
            ttk.Label(top, textvariable=var, style='Mono.TLabel',
                      width=10).pack(side=tk.LEFT, padx=(2, 0))

        ttk.Frame(top, width=10).pack(side=tk.LEFT)
        ttk.Button(top, text="Teleport", style='Action.TButton',
                   command=self._map_teleport).pack(side=tk.LEFT, padx=(0, 4))
        ttk.Button(top, text="Set Height", style='Action.TButton',
                   command=self._map_set_height).pack(side=tk.LEFT, padx=(0, 4))

        # Zoom controls on the right
        ttk.Button(top, text="Reset", style='Small.TButton',
                   command=lambda: self._map_set_zoom(1.0)).pack(side=tk.RIGHT, padx=(2, 0))
        ttk.Button(top, text="-", width=3, style='Arrow.TButton',
                   command=lambda: self._map_set_zoom(
                       self._map_zoom / 1.5)).pack(side=tk.RIGHT, padx=(2, 0))
        ttk.Button(top, text="+", width=3, style='Arrow.TButton',
                   command=lambda: self._map_set_zoom(
                       self._map_zoom * 1.5)).pack(side=tk.RIGHT, padx=(2, 0))
        self._map_zoom_var = tk.StringVar(value="1.0x")
        ttk.Label(top, textvariable=self._map_zoom_var,
                  font=('Consolas', 9)).pack(side=tk.RIGHT, padx=(0, 4))

        # Canvas for the map
        self._map_canvas = tk.Canvas(win, bg='#1a1a2e',
                                     highlightthickness=0, cursor='crosshair')
        self._map_canvas.pack(fill=tk.BOTH, expand=True, padx=8, pady=(0, 4))

        # Status line under map
        self._map_status_var = tk.StringVar(value="Click to set destination — right-click drag to pan — scroll to zoom")
        ttk.Label(win, textvariable=self._map_status_var,
                  style='Dim.TLabel').pack(anchor='w', padx=8, pady=(0, 8))

        # Bind events
        self._map_canvas.bind('<Configure>', lambda _: self._refresh_map())
        self._map_canvas.bind('<Button-1>', self._map_clicked)
        self._map_canvas.bind('<MouseWheel>', self._map_scroll)
        # Right-click drag to pan
        self._map_canvas.bind('<Button-3>', self._map_drag_start)
        self._map_canvas.bind('<B3-Motion>', self._map_drag_motion)

        # Load the map image
        self._load_map_image()

    def _toggle_map(self):
        """Open or close the world map window."""
        if self._map_visible and self._map_window:
            self._map_window.destroy()
            self._map_window = None
            self._map_visible = False
            self._map_btn.configure(text="World Map \u25B6")
        else:
            self._build_map_window()
            ...
            my = self.winfo_y()
            self._map_window.geometry(f"+{mx}+{my}")
            self.after(50, self._refresh_map)

    def _load_map_image(self):
        """Find and load worldmap.jpg/png from next to the script."""
        search_dirs = [
            os.path.dirname(os.path.abspath(__file__)),
            os.getcwd(),
        ]
        for d in search_dirs:
            for fname in ("worldmap.jpg", "worldmap.png"):
                p = os.path.join(d, fname)
                if os.path.isfile(p):
                    self._map_image = Image.open(p)
                    self._map_status_var.set(
                        f"Map loaded ({self._map_image.width}x{self._map_image.height})"
                        " — click to set destination, scroll to zoom")
                    self.after(100, self._refresh_map)
                    return
        self._map_status_var.set(
            "worldmap.jpg not found — place it next to cd_teleporter.py")

    def _game_to_norm(self, gx, gz):
        """Convert game world coords to normalized 0..1 coords."""
        b = WORLD_BOUNDS
        nx = (gx - b["x_min"]) / (b["x_max"] - b["x_min"])
        nz = (gz - b["z_min"]) / (b["z_max"] - b["z_min"])
        return nx, 1.0 - nz  # flip Z so north = up

    def _norm_to_game(self, nx, ny):
        """Convert normalized 0..1 coords back to game world coords."""
        b = WORLD_BOUNDS
        gx = b["x_min"] + nx * (b["x_max"] - b["x_min"])
        gz = b["z_min"] + (1.0 - ny) * (b["z_max"] - b["z_min"])
        return gx, gz

    def _refresh_map(self):
        """Redraw the map canvas with current zoom, player dot, and destination."""
        ...
        img_w, img_h = self._map_image.size
        zoom = self._map_zoom

        # Determine the crop region in image pixel coords
        if zoom > 1.0 and self._map_center:
            cx_n, cy_n = self._game_to_norm(*self._map_center)
            cx_px = cx_n * img_w
            cy_px = cy_n * img_h
            vw = img_w / zoom
            vh = img_h / zoom
            x0 = max(0, min(cx_px - vw / 2, img_w - vw))
            y0 = max(0, min(cy_px - vh / 2, img_h - vh))
        else:
            x0, y0 = 0, 0
            vw, vh = img_w, img_h

        # Crop and resize to fit canvas (maintain aspect ratio)
        crop_box = (int(x0), int(y0), int(x0 + vw), int(y0 + vh))
        cropped = self._map_image.crop(crop_box)

        # Fit to canvas preserving aspect ratio
        aspect = vw / vh
        if cw / ch > aspect:
            disp_h = ch
            disp_w = int(ch * aspect)
        else:
            disp_w = cw
            disp_h = int(cw / aspect)

        resized = cropped.resize((disp_w, disp_h), Image.LANCZOS)

        # Draw overlay dots using PIL (avoids canvas item flickering)
        draw = ImageDraw.Draw(resized)

        def _game_to_disp(gx, gz):
            """Game coords -> pixel on the resized display image."""
            nx, ny = self._game_to_norm(gx, gz)
            # Convert to crop-relative
            rx = (nx * img_w - x0) / vw * disp_w
            ry = (ny * img_h - y0) / vh * disp_h
            return rx, ry
        ...
        if self.engine.attached and self.engine.hooks_installed:
            ...
            if apos:
                px, py = _game_to_disp(apos[0], apos[2])
                r = 6
                if 0 <= px < disp_w and 0 <= py < disp_h:
                    draw.ellipse([px - r, py - r, px + r, py + r],
                                 fill='#ff1e1e', outline='white', width=2)
                    draw.text((px + r + 3, py - 8), "You",
                              fill='white')

        # Destination (blue dot)
        if self._map_dest:
            dx, dy_, dz = self._map_dest
            bx, by = _game_to_disp(dx, dz)
            r = 5
            if 0 <= bx < disp_w and 0 <= by < disp_h:
                draw.ellipse([bx - r, by - r, bx + r, by + r],
                             fill='#3278ff', outline='white', width=2)
                draw.text((bx + r + 3, by - 8),
                          f"({dx:.0f}, {dz:.0f})", fill='#c8dcff')

        # Waypoint markers (small yellow dots)
        for wp in self.waypoints.local:
            wx, wz = wp.get("absX", 0), wp.get("absZ", 0)
            if wx == 0 and wz == 0:
                continue
            mx, my = _game_to_disp(wx, wz)
            if 0 <= mx < disp_w and 0 <= my < disp_h:
                r = 3
                draw.ellipse([mx - r, my - r, mx + r, my + r],
                             fill='#f9e2af', outline='#b0a088')
        ...
        off_x = (cw - disp_w) // 2
        off_y = (ch - disp_h) // 2
        canvas.create_image(off_x, off_y, anchor='nw', image=self._map_photo)
        # Store display geometry for click conversion
        self._map_disp = (off_x, off_y, disp_w, disp_h, x0, y0, vw, vh)

    def _map_set_zoom(self, z):
        self._map_zoom = max(1.0, min(z, 8.0))
        self._map_zoom_var.set(f"{self._map_zoom:.1f}x")
        # If no center set, use player position or map center
        if self._map_center is None:
            if self.engine.attached:
                apos = self.engine.get_player_abs()
                if apos:
                    self._map_center = (apos[0], apos[2])
            if self._map_center is None:
                b = WORLD_BOUNDS
                self._map_center = ((b["x_min"] + b["x_max"]) / 2,
                                    (b["z_min"] + b["z_max"]) / 2)
        if z <= 1.0:
    ...
    def _map_scroll(self, event):
        if event.delta > 0:
            self._map_set_zoom(self._map_zoom * 1.25)
        elif event.delta < 0:
            self._map_set_zoom(self._map_zoom / 1.25)

    def _map_click_to_game(self, event):
        """Convert a canvas click event to game (x, z) coords. Returns None if outside."""
        if self._map_disp is None:
            return None
        off_x, off_y, disp_w, disp_h, crop_x0, crop_y0, crop_w, crop_h = self._map_disp
        # Click relative to the displayed image
        cx = event.x - off_x
        cy = event.y - off_y
        if not (0 <= cx < disp_w and 0 <= cy < disp_h):
            return None
        img_w, img_h = self._map_image.size
        # Map to full-image pixel coords
        src_x = crop_x0 + (cx / disp_w) * crop_w
        src_y = crop_y0 + (cy / disp_h) * crop_h
        # Normalize
        nx = src_x / img_w
        ny = src_y / img_h
        return self._norm_to_game(nx, ny)

    def _map_clicked(self, event):
        result = self._map_click_to_game(event)
        if result is None:
            return
        gx, gz = result
        self._map_dest = (gx, self._map_height, gz)
        self._map_dest_x_var.set(f"{gx:.1f}")
        self._map_dest_z_var.set(f"{gz:.1f}")
        self._map_center = (gx, gz)
        self._map_status_var.set(
            f"Destination: ({gx:.1f}, {gz:.1f}) Height: {self._map_height:.0f}"
            f" — click Teleport to go")
        self._refresh_map()

    def _map_drag_start(self, event):
        """Begin right-click drag to pan the map."""
        self._map_drag_last = (event.x, event.y)
        # Auto-set a center if we don't have one yet
        if self._map_center is None:
            b = WORLD_BOUNDS
            self._map_center = ((b["x_min"] + b["x_max"]) / 2,
                                (b["z_min"] + b["z_max"]) / 2)
        # Auto-zoom in slightly if at 1x so panning has effect
        if self._map_zoom < 1.5:
            self._map_set_zoom(2.0)

    def _map_drag_motion(self, event):
        """Pan the map by dragging with right mouse button."""
        if not hasattr(self, '_map_drag_last') or self._map_disp is None:
            return
        ...
        # Convert pixel drag to game-coord shift
        _, _, disp_w, disp_h, _, _, crop_w, crop_h = self._map_disp
        img_w, img_h = self._map_image.size
        b = WORLD_BOUNDS
        # Pixels dragged -> fraction of crop -> game units
        game_dx = -(dx / disp_w) * (crop_w / img_w) * (b["x_max"] - b["x_min"])
        game_dz = (dy / disp_h) * (crop_h / img_h) * (b["z_max"] - b["z_min"])

        cx, cz = self._map_center
        self._map_center = (cx + game_dx, cz + game_dz)
        self._refresh_map()

    def _map_teleport(self):
        """Teleport to the destination set via the map."""
        ...
        if ok:
            ...
            self._map_status_var.set(
                f"Teleported to ({x:.1f}, {z:.1f}) Height: {y:.0f}")
            self.bottom_var.set(
                f"Map teleport to ({x:.1f}, {z:.1f})")
        else:
            self.bottom_var.set(err)

    def _map_set_height(self):
        """Open dialog to set the teleport height for map destinations."""
        dlg = tk.Toplevel(self._map_window or self)
        dlg.title("Set Teleport Height")
        dlg.configure(bg=self.BG)
        dlg.resizable(False, False)
        dlg.transient(self._map_window or self)
        dlg.grab_set()

        parent = self._map_window or self
        dlg.update_idletasks()
        px = parent.winfo_x() + (parent.winfo_width() - 380) // 2
        py = parent.winfo_y() + (parent.winfo_height() - 250) // 2
        dlg.geometry(f"380x250+{px}+{py}")

        # Info text
        tk.Label(dlg, text="Map height is not automatically derived\nfrom map data.",
                 bg=self.BG, fg=self.FG_DIM, font=('Segoe UI', 9),
                 justify='center').pack(padx=16, pady=(16, 12))

        # Custom height input
        input_row = tk.Frame(dlg, bg=self.BG)
        input_row.pack(fill=tk.X, padx=16, pady=(0, 12))
        tk.Label(input_row, text="Height:", bg=self.BG, fg=self.FG,
                 font=('Segoe UI', 10)).pack(side=tk.LEFT)
        height_var = tk.StringVar(value=f"{self._map_height:.0f}")
        height_entry = tk.Entry(input_row, textvariable=height_var,
                                bg=self.BG_INPUT, fg=self.FG,
                                insertbackground=self.FG, relief='flat',
                                font=('Consolas', 11), width=10,
                                highlightbackground=self.BORDER,
                                highlightcolor=self.ACCENT, highlightthickness=1)
        height_entry.pack(side=tk.LEFT, padx=(8, 0), fill=tk.X, expand=True)
        height_entry.select_range(0, tk.END)
        height_entry.focus_set()

        def _apply(val):
            try:
                self._map_height = float(val)
            except ValueError:
                return
            self._map_height_var.set(f"{self._map_height:.0f}")
            if self._map_dest:
                x, _, z = self._map_dest
                self._map_dest = (x, self._map_height, z)
            # Persist to settings
            settings = _load_settings()
            settings["map_height"] = self._map_height
            _save_settings(settings)
            dlg.destroy()

        # Preset buttons
        preset_row = tk.Frame(dlg, bg=self.BG)
        preset_row.pack(fill=tk.X, padx=16, pady=(0, 12))
        self._make_primary_btn(preset_row, "Ground (1200)",
                               lambda: _apply("1200")).pack(
            side=tk.LEFT, padx=(0, 8), fill=tk.X, expand=True)
        self._make_primary_btn(preset_row, "Abyss (2500)",
                               lambda: _apply("2500")).pack(
            side=tk.LEFT, fill=tk.X, expand=True)

        # OK / Cancel
        btn_row = tk.Frame(dlg, bg=self.BG)
        btn_row.pack(fill=tk.X, padx=16, pady=(0, 12))
        self._make_primary_btn(btn_row, "OK",
                               lambda: _apply(height_var.get())).pack(
            side=tk.RIGHT, padx=(4, 0))
        self._make_secondary_btn(btn_row, "Cancel",
                                 dlg.destroy).pack(side=tk.RIGHT)

        height_entry.bind('<Return>', lambda _: _apply(height_var.get()))
    ...
    def _auto_attach(self):
        try:
            ...
            self._set_status("Error", self.ERR_CLR)
            self.bottom_var.set(str(e))
    ...
    def _poll(self):
        ...
        if self.engine.attached and self.engine.hooks_installed:
            ...
            # Refresh world map window (throttled to ~every 500ms)
            if _HAS_PIL and self._map_visible and self._map_image:
                now = time.time()
    ...
    def _teleport_to_map(self):
        ...
        if y == 0.0:
            self.bottom_var.set(
                "Invalid map marker (Y=0). Must select a map object.")
            return
        apos = self.engine.get_player_abs()
        if apos:
            self.recovery_pos = apos
        y += HEIGHT_BOOST
        ok, err = self.engine.teleport_to_abs(x, y, z)
        if ok:
            self._trigger_invuln()
            self.bottom_var.set(
                f"Teleported to map marker ({x:.0f}, {y:.0f}, {z:.0f})")
        else:
            self.bottom_var.set(err)
