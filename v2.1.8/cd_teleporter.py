...
def _is_main_run():
    ...
    if os.environ.get("_CD_WEBMAP_CHILD"):
        return False
    return True
...
def _point_on_any_monitor(x, y):
    """Return True if (x, y) lies inside the virtual-screen bounding rect
    (the union of all connected monitors). Uses GetSystemMetrics rather
    than MonitorFromPoint — passing structs by value through ctypes
    doesn't reliably honour MONITOR_DEFAULTTONULL on Windows x64, and
    silently returns the primary HMONITOR for any input. The virtual
    screen rect is a slight over-approximation in L-shaped multi-monitor
    layouts, but it correctly rejects the (-32000, -32000) sentinel and
    other off-screen coordinates, which is what we care about here.
    Defined at module scope so subprocess save callbacks can use it too."""
    try:
...
def _scrub_window_coords(data):
    """Drop x/y from any saved window block whose coords aren't on a
    connected monitor. Mutates `data` in place. Returns True if anything
    changed (caller may want to persist)."""
    changed = False
    for key in ("window", "webmap_window"):
        block = data.get(key)
        ...
        if x is None or y is None:
            continue
        # Check a point inset from the top-left so a window with its
        # title bar 1 px off-screen isn't rejected unnecessarily.
        if not _point_on_any_monitor(x + 50, y + 25):
            block.pop("x", None)
            block.pop("y", None)
            changed = True
    return changed
...
def _load_settings():
    try:
        ...
        return {}
    # Scrub bad window coords on every load and persist immediately if any
    # were cleaned, so corrupt state from older builds (e.g. -32000 sentinel
    # from closing while minimized) self-heals on first launch and never
    # comes back even if subsequent close paths fail to save valid coords.
    if isinstance(data, dict) and _scrub_window_coords(data):
        try:
            _save_settings(data)
        except Exception:
            pass
...
class TeleportEngine:
    ...
    # 2.1.6: game switched entity/map writes to VEX (AVX) encoding.
    AOB_ENTITY = b'\x48\x8B\x06\xC5\xF8\x11\x88\xB0\x01\x00\x00'  # VEX vmovups [rax+1B0], xmm1
    AOB_POS    = b'\x0F\x11\x99\x90\x00\x00\x00'                  # SSE movups [rcx+90], xmm3
    AOB_HEALTH = b'\x48\x8B\x46\x08\x48\x89\xF1'
    # Map marker: v1.13 has several generic 12-byte vector-copy matches. This
    # longer anchor targets the destination marker write that updates RBX+0x20
    # and then checks the RBX+0x3C initialized flag. Hook site is still +4 into
    # the AOB (at the vmovsd store).
    AOB_MAP    = (b'\xC5\xFB\x10\x07\xC5\xFB\x11\x02\x8B\x47\x08\x89\x42\x08'
                  b'\x80\x7B\x3C\x00\x75\x0D\x4C\x8D\x43\x10')
    # World offset in 2.1.6+ is loaded via VEX vsubps xmm0, xmm0, [rip+disp32].
    # The instruction appears 2+ times in the module, all pointing to the same
    # world-origin data block (16 bytes of floats). We scan all hits, compute
    # each RIP-relative target, and pick the most-referenced one.
    AOB_WORLD  = b'\xC5\xF8\x5C\x05'

    # Length of the instruction(s) each hook replaces with the JMP patch.
    # Entity (hook_a): VEX vmovups [rax+1B0], xmm1 = 8 bytes (was 7 SSE pre-2.1.6)
    # Position (hook_b): SSE movups [rcx+90], xmm3 = 7 bytes
    # Health (hook_c):   mov rax,[rsi+8] ; mov rcx,rsi = 4 + 3 = 7 bytes
    # Map (hook_d):      VEX vmovsd [rdx],xmm0 ; mov eax,[rdi+8] = 4 + 3 = 7 bytes
    HOOK_A_LEN = 8
    HOOK_B_LEN = 7
    HOOK_C_LEN = 7
    HOOK_D_LEN = 7
    ...
    def scan_and_hook(self):
        ...
        self.hook_d = base + idx + 4

        # World offset: scan for VEX `vsubps xmm0, xmm0, [rip+disp32]` hits,
        # compute each RIP-relative target, and keep the one referenced most
        # frequently. That's the static 16-byte world-origin block used by the
        # game's world→local coord conversion (confirmed via CE tracing a
        # write to the local position in the 2026-04 build).
        targets = Counter()
        pos = 0
        while pos < len(data) - 8:
            ...
            targets[base + i + 8 + disp] += 1
            pos = i + 1
        if targets:
            self.world_offset_addr = targets.most_common(1)[0][0]
    ...
    def _jmp_patch(self, from_addr, to_addr, replaced_len=7):
        """Build a JMP patch that fills `replaced_len` bytes (>=5)."""
        if replaced_len < 5:
            raise ValueError("replaced_len must be >= 5 for a rel32 jump")
        return (b'\xE9' + struct.pack('<i', self._rel32(from_addr, to_addr))
                + b'\x90' * (replaced_len - 5))
    ...
    def _build_cave_a(self):
        ...
        td = self.td
        ret = self.hook_a + self.HOOK_A_LEN
        c = bytearray()
        c += b'\x51'                                            # push rcx
        c += b'\x48\xB9' + struct.pack('<Q', td)                # mov rcx, teleportData
        c += b'\x48\x89\x41\x18'                                # mov [rcx+18], rax
        # Read player position (x/y/z floats at +0x90/+0x94/+0x98) into tele block.
        # Use VEX encoding to avoid SSE/AVX state transition penalty on AVX-heavy
        # code paths in 2.1.6+ (the game now uses VEX in this function).
        c += b'\xC5\xFA\x10\x80\x90\x00\x00\x00'               # vmovss xmm0, [rax+90]
        c += b'\xC5\xFA\x11\x41\x20'                            # vmovss [rcx+20], xmm0
        c += b'\xC5\xFA\x10\x80\x94\x00\x00\x00'               # vmovss xmm0, [rax+94]
        c += b'\xC5\xFA\x11\x41\x24'                            # vmovss [rcx+24], xmm0
        c += b'\xC5\xFA\x10\x80\x98\x00\x00\x00'               # vmovss xmm0, [rax+98]
        c += b'\xC5\xFA\x11\x41\x28'                            # vmovss [rcx+28], xmm0
        c += b'\x83\x79\x10\x00'                                # cmp [rcx+10], 0
        c += b'\x7E\x07'                                        # jle skip (+7)
        c += b'\xC5\xF8\x10\x09'                                # vmovups xmm1, [rcx]  (4 bytes VEX)
        c += b'\xFF\x49\x10'                                    # dec [rcx+10]         (3 bytes)
        # skip:
        c += b'\x59'                                            # pop rcx
        # Re-emit original VEX vmovups [rax+1B0], xmm1 (8 bytes in 2.1.6+)
        c += b'\xC5\xF8\x11\x88\xB0\x01\x00\x00'
        c += self._abs_jmp(ret)
        return bytes(c)
    ...
    def _build_cave_b(self):
        ...
        td = self.td
        ret = self.hook_b + self.HOOK_B_LEN
        c = bytearray()
        c += b'\x50'                                            # push rax
    ...
    def _build_cave_c(self):
        ...
        inv = self.inv
        ret = self.hook_c + self.HOOK_C_LEN
        c = bytearray()
        c += b'\x53'                                            # push rbx
    ...
    def _build_cave_d(self):
        ...
        md = self.md
        ret = self.hook_d + self.HOOK_D_LEN
        c = bytearray()
        # Re-emit the original 7 bytes: VEX vmovsd [rdx],xmm0 ; mov eax,[rdi+8]
        c += b'\xC5\xFB\x11\x02'                                # vmovsd [rdx], xmm0  (VEX, was F2 0F 11 02)
    ...
    def _install_hooks(self):
        ...
        if not self._far_mode:
            ...
            for hook_addr, cave_addr, rlen in hooks:
                self.orig_bytes[hook_addr] = self.pm.read_bytes(hook_addr, rlen)
                patch = self._jmp_patch(hook_addr, cave_addr, rlen)
                self.pm.write_bytes(hook_addr, patch, rlen)
        else:
            ...
            for hook_addr, cave_addr, rlen in hooks:
                ...
                self.orig_bytes[hook_addr] = self.pm.read_bytes(hook_addr, rlen)
                patch = self._jmp_patch(hook_addr, tramp, rlen)
                self.pm.write_bytes(hook_addr, patch, rlen)
...
(function() {
    ...
    waitForMap(function(map) {
        ...
        function getScale() {
            var z = map.getZoom();
            var s = Math.pow(1.4, z - window.__cdBaseZoom) * 0.4;
        ...
        function reprojectAll() {
            ...
            for (var i = 0; i < commEntries.length; i++) {
                ...
                c.el.style.transform = 'translate(' + cp.x + 'px,' + cp.y + 'px) scale(' + s + ')';
            }
        }
        ...
        playerEl.style.cssText = 'position:absolute;left:0;top:0;pointer-events:none;display:none;transform-origin:0 0;';
        playerEl.innerHTML =
        ...
        destEl.style.cssText = 'position:absolute;left:0;top:0;display:none;cursor:pointer;pointer-events:auto;transform-origin:0 0;';
        destEl.innerHTML =
            '<svg viewBox="0 0 100 100" width="36" height="36" style="position:absolute;left:-18px;top:-18px;filter:drop-shadow(0 0 3px rgba(0,0,0,0.5));">'
            + '<path d="M18 82 C20 78, 28 62, 38 52 C42 48, 46 46, 50 44'
            + ' C44 38, 30 22, 22 14 C20 12, 19 10, 20 9'
            + ' C21 8, 23 9, 26 12 C34 20, 46 36, 52 44'
            + ' C56 38, 62 28, 68 20 C72 15, 76 10, 78 8'
            + ' C80 7, 81 8, 80 10'
            + ' C78 14, 70 28, 62 40 C58 45, 55 48, 52 50'
            + ' C58 56, 68 70, 76 80'
            + ' C78 83, 80 86, 78 87'
            + ' C76 88, 74 86, 72 83'
            + ' C64 72, 56 60, 50 52'
            + ' C44 60, 32 74, 24 84'
            + ' C22 86, 18 86, 18 82 Z"'
            + ' fill="#c43c2d" stroke="#8b1a0e" stroke-width="1"/>'
            + '</svg>'
            + '<div style="position:absolute;left:22px;top:-8px;color:#ffa07a;font:11px Consolas,monospace;'
            + 'text-shadow:0 0 3px #000;white-space:nowrap;"></div>';
        destEl.addEventListener('click', function(e) {
            e.stopPropagation();
            window.__cdClearDest();
            if (window.pywebview && window.pywebview.api) {
                window.pywebview.api.on_dest_cleared();
            }
        });
        ...
        window.__cdUpdateWaypoints = function(waypointsJson) {
            ...
            waypoints.forEach(function(wp) {
                ...
                el.style.cssText = 'position:absolute;left:0;top:0;cursor:pointer;pointer-events:auto;transform-origin:0 0;';
                el.innerHTML =
                    ...
                    + 'text-shadow:0 0 3px #000,0 0 3px #000;white-space:nowrap;pointer-events:none;">' + (wp.name || '') + '</div>';
                (function(idx) {
        ...
        // --- Community waypoint markers (cyan dots) ---
        var commEntries = [];  // [{el, lnglat}]

        window.__cdUpdateCommunityWaypoints = function(waypointsJson) {
            var waypoints = JSON.parse(waypointsJson);
            for (var i = 0; i < commEntries.length; i++) {
            ...
            waypoints.forEach(function(wp) {
                ...
                (function(idx) {
                    el.addEventListener('click', function(e) {
                        e.stopPropagation();
                        if (window.pywebview && window.pywebview.api) {
                            window.pywebview.api.on_comm_waypoint_clicked(idx);
                        }
                    });
                })(wp.idx);
                overlay.appendChild(el);
        ...
        // --- Focused community waypoint marker (cyan, single) ---
        var focusEl = document.createElement('div');
        focusEl.id = 'cd-focus-marker';
        focusEl.style.cssText = 'position:absolute;left:0;top:0;display:none;pointer-events:none;transform-origin:0 0;';
        focusEl.innerHTML =
            '<div style="position:absolute;left:-7px;top:-7px;width:14px;height:14px;'
            + 'background:#a8d4f0;border:1.5px solid #fff;border-radius:50%;'
            + 'box-shadow:0 0 6px rgba(168,212,240,0.6);"></div>'
            + '<div style="position:absolute;left:0;top:-20px;color:#a8d4f0;font:12px Segoe UI,sans-serif;'
            + 'text-shadow:0 0 3px #000,0 0 3px #000;white-space:nowrap;"></div>';
        overlay.appendChild(focusEl);
        var focusLngLat = null;
        var focusVisible = false;
        var focusLabel = focusEl.children[1];

        // Add focus marker to reproject loop
        var origReproject = reprojectAll;
        reprojectAll = function() {
            origReproject();
            if (focusVisible && focusLngLat) {
                var s = getScale();
                var f = map.project(focusLngLat);
                focusEl.style.transform = 'translate(' + f.x + 'px,' + f.y + 'px) scale(' + s + ')';
            }
        };

        window.__cdShowFocusMarker = function(lng, lat, name) {
            focusLngLat = [lng, lat];
            focusVisible = true;
        ...
        window.__cdHideFocusMarker = function() {
            focusVisible = false;
...
def _run_webmap_process(pipe_fd, mapgenie_url, inject_js, icon_path=None,
                        win_x=None, win_y=None, win_w=1400, win_h=900):
    ...
    class Bridge:
        ...
        def on_dest_cleared(self):
            state["dest_lnglat"] = None
            try:
                print(json.dumps({"type": "dest_clear"}), flush=True)
            except Exception:
                pass
        ...
        def on_comm_waypoint_clicked(self, idx):
            try:
                print(json.dumps({"type": "comm_waypoint_click", "idx": idx}), flush=True)
            except Exception:
                pass
    ...
    bridge = Bridge()
    # Persist browser profile (cookies/login) so MapGenie stays logged in
    webview_storage = os.path.join(
        os.environ.get("LOCALAPPDATA", ""), "CD_Teleport", "webview_data")

    win_kwargs = {
        "title": 'Crimson Desert \u2014 MapGenie Map',
        "url": mapgenie_url,
        "js_api": bridge,
        "width": win_w,
        "height": win_h,
    ...
    if win_x is not None and win_y is not None:
        win_kwargs["x"] = win_x
        win_kwargs["y"] = win_y
    win = wv.create_window(**win_kwargs)
    state["win"] = win

    # Intercept links/popups that would open in external browser so that
    # MapGenie login/OAuth flows stay inside the webview.
    _LOGIN_INTERCEPT_JS = r"""
    (function() {
        // Redirect window.open into the current window
        window.open = function(url) {
            if (url) window.location.href = url;
            return window;
        };
        // Rewrite target="_blank" links to navigate in-place
    ...
    def _on_loaded():
        try:
            win.evaluate_js(_LOGIN_INTERCEPT_JS)
            ...
            for line in pipe_in:
                ...
                if cmd == "update_player" and state["overlay_ready"]:
                    ...
                    try:
                        ...
                        pass
                elif cmd == "update_community_waypoints" and state["overlay_ready"]:
                    wp_json = json.dumps(msg["waypoints"]).replace("'", "\\'")
                    try:
                        win.evaluate_js(
                            f"window.__cdUpdateCommunityWaypoints && "
                            f"window.__cdUpdateCommunityWaypoints('{wp_json}')")
                    except Exception:
                        pass
                    ...
                    try:
                        ...
                        pass
                elif cmd == "show_focus" and state["overlay_ready"]:
                    lng = msg["lng"]
                    lat = msg["lat"]
                    name = msg.get("name", "").replace("'", "\\'")
                    try:
                        win.evaluate_js(
                            f"window.__cdShowFocusMarker && window.__cdShowFocusMarker("
                            f"{lng}, {lat}, '{name}')")
                    except Exception:
                        pass
                elif cmd == "hide_focus" and state["overlay_ready"]:
                    try:
                        win.evaluate_js(
                            "window.__cdHideFocusMarker && window.__cdHideFocusMarker()")
                    except Exception:
                        pass
                elif cmd == "close":
                    try:
    ...
    def _set_window_icon():
        ...
        try:
            ...
            def _enum_cb(hwnd, _lp):
                ...
                return True  # continue — set icon on ALL our windows
            user32.EnumWindows(WNDENUMPROC(_enum_cb), 0)
        except Exception:
            pass

    def _on_closing():
        """Save window geometry directly to settings before closing.

        Guard against bad coordinates from any cause:
          - The (-32000, -32000) sentinel Windows reports for minimized
            windows (pywebview just passes that through).
          - Coordinates on a now-disconnected monitor (multi-monitor users).
          - Anything else that lands the window off every connected display.
        Validation: MonitorFromPoint on a point near the title bar (saved
        x+50, y+25). If that point isn't on any monitor, fall back to the
        previously persisted position; if THAT is also invalid, write the
        size only and leave x/y unset so the next launch centers.
        """
        def _on_visible_monitor(px, py):
            return _point_on_any_monitor(px, py)

        try:
            settings = _load_settings()
            prev = settings.get("webmap_window", {}) or {}
            x, y = win.x, win.y
            w, h = win.width, win.height

            def _coords_valid(cx, cy):
                return (cx is not None and cy is not None
                        and _on_visible_monitor(cx + 50, cy + 25))

            if not _coords_valid(x, y):
                # Current coords are bad — try the previously saved ones.
                px, py = prev.get("x"), prev.get("y")
                if _coords_valid(px, py):
                    x, y = px, py
                    w = prev.get("width", w)
                    h = prev.get("height", h)
                else:
                    # Both current and previous are bad — drop x/y entirely
                    # so the next launch falls back to centering.
                    x, y = None, None
                    if w is None or h is None or w <= 0 or h <= 0:
                        w = prev.get("width", 1400)
                        h = prev.get("height", 900)

            entry = {"width": w, "height": h}
            if x is not None and y is not None:
                entry["x"] = x
                entry["y"] = y
            settings["webmap_window"] = entry
    ...
    def _on_shown():
        # Delay to ensure the window handle is fully ready, then
        # retry a few times in case the HWND isn't enumerable yet
        import time
        for _ in range(3):
            time.sleep(0.5)
            _set_window_icon()
    ...
    wv.start(func=_start_reader, debug=False, private_mode=False,
...
class TeleporterApp(tk.Tk):
    ...
    def __init__(self):
        ...
        # Restore window placement after UI is fully built
    ...
    def _build_ui(self):
        ...
        self.shared_tree.bind('<<TreeviewSelect>>', self._on_shared_waypoint_selected)
        ...
        map_title = ttk.Label(adv_inner, text="Map Marker Visibility",
                              style='Card.TLabel',
                              font=('Segoe UI', 10, 'bold'))
        map_title.pack(anchor='w', padx=12, pady=(8, 2))
        ttk.Label(adv_inner,
                  text="Control which waypoint markers appear on the MapGenie map.",
                  style='Card.TLabel',
                  font=('Segoe UI', 8)).pack(anchor='w', padx=12, pady=(0, 8))

        self._show_local_markers_var = tk.BooleanVar(value=True)
        local_mk_row = ttk.Frame(adv_inner, style='Card.TFrame')
        local_mk_row.pack(fill=tk.X, padx=12, pady=(0, 4))
        ttk.Checkbutton(local_mk_row, text="Show local waypoints on map",
                         variable=self._show_local_markers_var,
                         command=self._on_marker_visibility_changed).pack(
            side=tk.LEFT)

        self._show_community_markers_var = tk.BooleanVar(value=False)
        comm_mk_row = ttk.Frame(adv_inner, style='Card.TFrame')
        comm_mk_row.pack(fill=tk.X, padx=12, pady=(0, 8))
        ttk.Checkbutton(comm_mk_row,
                         text="Show community waypoints on map",
                         variable=self._show_community_markers_var,
                         command=self._on_marker_visibility_changed).pack(
            side=tk.LEFT)
    ...
    def _on_shared_waypoint_selected(self, _event=None):
        """When a community waypoint is selected, show it on the map and pan to it."""
        if not self._webmap_open or not self._webmap_overlay_ready:
            return
        ...
        try:
            ...
            self._webmap_pipe.write(json.dumps({
                "cmd": "show_focus", "lng": lng, "lat": lat,
                "name": loc.get("name", "")
            }) + "\n")
            self._webmap_pipe.write(json.dumps({
                "cmd": "pan_to", "lng": lng, "lat": lat
    ...
    def _hide_focus_marker(self):
        """Hide the community waypoint focus marker on the map."""
        if not hasattr(self, '_webmap_pipe') or not self._webmap_pipe:
            return
        try:
            self._webmap_pipe.write(json.dumps({"cmd": "hide_focus"}) + "\n")
            self._webmap_pipe.flush()
        except Exception:
            pass
    ...
    def _open_webmap(self):
        ...
        # Load map height and saved webmap geometry from settings
        settings = _load_settings()
        self._map_height = settings.get("map_height", 1200.0)
        self._webmap_dest = None
        self._webmap_overlay_ready = False
        wm_geo = settings.get("webmap_window", {})
        wm_x = wm_geo.get("x")
        wm_y = wm_geo.get("y")
        wm_w = wm_geo.get("width", 1400)
        wm_h = wm_geo.get("height", 900)

        # Rescue users carrying a bad saved position (e.g. the
        # (-32000, -32000) sentinel saved by closing while minimized).
        # If the saved point isn't on any connected monitor, drop it AND
        # rewrite settings now so the bad state doesn't persist across
        # program restarts where the child's save callback didn't run.
        if wm_x is not None and wm_y is not None:
            if not self._point_on_visible_monitor(wm_x + 50, wm_y + 25):
                wm_x = None
                wm_y = None
                cleaned = {"width": wm_w, "height": wm_h}
                settings["webmap_window"] = cleaned
                _save_settings(settings)
        ...
        icon_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'teleporter.ico')

        child_kwargs = {
            "pipe_fd": read_handle,
            "mapgenie_url": MAPGENIE_URL,
            "inject_js": MAPGENIE_INJECT_JS,
            "icon_path": icon_path,
            "win_x": wm_x, "win_y": wm_y,
            "win_w": wm_w, "win_h": wm_h,
        }

        if getattr(sys, 'frozen', False):
            # Frozen EXE: re-launch ourselves with env vars to signal
            # the child entry point
            child_env["_CD_WEBMAP_CHILD_ARGS"] = json.dumps(child_kwargs)
            ...
            # Script mode: launch Python with -c to import and call
            self._webmap_proc = subprocess.Popen(
                [sys.executable, "-c",
                 ...
                 f"{icon_path!r}, "
                 f"win_x={wm_x!r}, win_y={wm_y!r}, win_w={wm_w!r}, win_h={wm_h!r})"],
                stdout=subprocess.PIPE,
                stderr=subprocess.DEVNULL,
                close_fds=False,
                creationflags=0,
        ...
        def _read_child():
            try:
                for line in self._webmap_proc.stdout:
                    ...
                    elif mtype == "comm_waypoint_click":
                        idx = int(msg.get("idx", -1))
                        self.after(0, lambda i=idx: self._webmap_teleport_comm_waypoint(i))
                    elif mtype == "follow":
    ...
    def _close_webmap(self):
        ...
        if hasattr(self, '_webmap_pipe') and self._webmap_pipe:
            try:
                ...
                pass
        # Give the child time to save geometry and shut down gracefully
        if hasattr(self, '_webmap_proc') and self._webmap_proc:
            try:
                self._webmap_proc.wait(timeout=3)
            except Exception:
    ...
    def _update_webmap_overlay(self):
        ...
        try:
            ...
            if now - getattr(self, '_webmap_wp_update', 0) > 5.0:
                ...
                # Local waypoints
                wp_data = []
                if self._show_local_markers_var.get():
                ...
                # Community waypoints
                comm_data = []
                if self._show_community_markers_var.get():
                ...
                self._webmap_pipe.write(json.dumps({
                    "cmd": "update_community_waypoints",
                    "waypoints": comm_data
                }) + "\n")
                self._webmap_pipe.flush()
        except Exception:
    ...
    def _webmap_teleport_comm_waypoint(self, idx):
        """Teleport to a community waypoint clicked on the map overlay."""
        if idx < 0 or idx >= len(self.waypoints.shared):
            return
        if not self.engine.attached or not self.engine.hooks_installed:
            self.bottom_var.set("Not connected to game.")
            return
    ...
    def _load_adv_settings(self):
        ...
        self._show_local_markers_var.set(adv.get("show_local_markers", True))
        self._show_community_markers_var.set(adv.get("show_community_markers", False))
    ...
    def _save_adv_settings(self):
        ...
        settings["advanced"] = {
            ...
            "show_local_markers": self._show_local_markers_var.get(),
            "show_community_markers": self._show_community_markers_var.get(),
        }
        _save_settings(settings)
    ...
    def _on_marker_visibility_changed(self):
        """When map marker visibility checkboxes change, save and force refresh."""
        self._save_adv_settings()
        # Force waypoint refresh on next overlay update
        self._webmap_wp_update = 0
    ...
    def _point_on_visible_monitor(self, x, y):
        """Return True if (x, y) lies inside the virtual-screen rect."""
        return _point_on_any_monitor(x, y)

    def _restore_window_geometry(self):
        """Restore saved window position/size or centre on screen."""
        default_w, default_h = 760, 820
        settings = _load_settings()
        wg = settings.get("window", {})
        w = wg.get("width", default_w)
        h = wg.get("height", default_h)
        x = wg.get("x")
        y = wg.get("y")

        # Set size via geometry() (handles client area correctly)
        self.geometry(f"{w}x{h}")
        self.update_idletasks()

        # Validate the saved position is still on a connected monitor before
        # applying it. Two off-screen failure modes this guards against:
        #   1. The user moved the window onto a secondary monitor that has
        #      since been disconnected/rearranged.
        #   2. A previous version saved the (-32000, -32000) sentinel that
        #      Windows reports for minimized windows (fixed in _save_window_
        #      geometry below; this guard rescues users carrying that bad
        #      saved state forward from older versions).
        # Check a point near the title bar (offset in from top-left) so a
        # mostly-on-screen window isn't rejected for a 1-pixel overlap.
        check_x = (x + 50) if x is not None else None
        check_y = (y + 25) if y is not None else None
        position_valid = (
            x is not None
            and y is not None
            and self._point_on_visible_monitor(check_x, check_y)
        )

        if position_valid:
            # Use Win32 API for position only — Tk geometry() can't
            # handle absolute negative coordinates on multi-monitor
            try:
                hwnd = int(self.wm_frame(), 16)
                SWP_NOZORDER = 0x0004
                SWP_NOSIZE = 0x0001
                ctypes.windll.user32.SetWindowPos(
                    hwnd, 0, x, y, 0, 0, SWP_NOZORDER | SWP_NOSIZE)
            except Exception:
    ...
    def _save_window_geometry(self):
        ...
        Uses GetWindowPlacement (not GetWindowRect) so the *restored*
        position is captured even when the window is minimized at close
        time — minimized windows report rcWindow = (-32000, -32000) via
        GetWindowRect, which would otherwise be saved and cause the next
        launch to position off-screen.

        Position comes from rcNormalPosition (full window rect, screen
        coords). Size comes from winfo_width/height (client area), because
        Tk's geometry() restores client area. Mixing the two is critical:
        rcNormalPosition's width/height *includes* the title bar/borders,
        and saving that into geometry() would cause the window to grow by
        the decoration thickness on every save/restore cycle.
        """
        try:
            ...
            if not ctypes.windll.user32.GetWindowPlacement(
                ...
                return
            rn = wp.rcNormalPosition

            settings = _load_settings()
            prev = settings.get("window", {}) or {}

            SW_SHOWMINIMIZED = 2
            if wp.showCmd == SW_SHOWMINIMIZED:
                # Minimized: winfo_width/height is unreliable. Keep the
                # previously saved size and only update position.
                w = prev.get("width", 760)
                h = prev.get("height", 820)
            else:
                w = self.winfo_width()
                h = self.winfo_height()
...
if __name__ == "__main__":
    # When running as a frozen EXE, the webmap child is launched as
    # another copy of this EXE with _CD_WEBMAP_CHILD set.  Detect that
    # and run the webmap process directly instead of main().
    if os.environ.get("_CD_WEBMAP_CHILD") and getattr(sys, 'frozen', False):
        import json as _json_child
        _child_args = _json_child.loads(os.environ["_CD_WEBMAP_CHILD_ARGS"])
        _run_webmap_process(**_child_args)
    else:
        main()
