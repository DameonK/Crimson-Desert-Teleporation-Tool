...
:: Only install pymem if the selected interpreter cannot already import it.
"%PYTHON%" -c "import pymem" >nul 2>&1
if errorlevel 1 (
    echo pymem not found for this Python. Installing to local folder...
    ...
    "%PYTHON%" -m pip install pymem --target="%PY_LIBS%" --quiet --no-warn-script-location
    if errorlevel 1 (
        echo Failed to install pymem.
