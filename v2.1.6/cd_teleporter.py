...
class TeleporterApp(tk.Tk):
    ...
    def _save_window_geometry(self):
        ...
        try:
            # Save client area size (winfo) so it matches geometry() on
            # restore.  Save position from GetWindowRect so negative
            # multi-monitor coords are captured correctly.
            hwnd = int(self.wm_frame(), 16)
